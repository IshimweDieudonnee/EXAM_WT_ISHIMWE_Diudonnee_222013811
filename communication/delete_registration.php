...
function showDeleteConfirmation($ID) {
    ...
    function confirmDeletion(ID) {
        window.location.href = '?registration_id=' + ID + '&confirm=yes';
    }
    function returnToProduct() {
        window.location.href = 'registrationhp';
    }
    </script>
HTML;
}
...
if(isset($_REQUEST['registration_id'])) {
    ...
    if (isset($_REQUEST['confirm']) && $_REQUEST['confirm'] == 'yes') {
        ...
        $stmt = $connection->prepare("DELETE FROM registration WHERE registration_id=?");
        $stmt->bind_param("i", $ID);
        if ($stmt->execute()) {
            echo "<script>alert('Record deleted successfully.'); window.location.href = 'registrationhp';</script>";
        } else {
    ...
    echo "<script>alert('ID is not set.'); window.location.href = 'registrationhp';</script>";
}
