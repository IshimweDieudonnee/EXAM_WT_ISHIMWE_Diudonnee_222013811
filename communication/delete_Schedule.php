...
function showDeleteConfirmation($ID) {
    ...
    function confirmDeletion(ID) {
        window.location.href = '?schedule_id=' + ID + '&confirm=yes';
    }
...
if(isset($_REQUEST['schedule_id'])) {
    ...
    if (isset($_REQUEST['confirm']) && $_REQUEST['confirm'] == 'yes') {
        ...
        $stmt = $connection->prepare("DELETE FROM workshopschedule WHERE schedule_id=?");
        $stmt->bind_param("i", $ID);
        if ($stmt->execute()) {
            echo "<script>alert('Record deleted successfully.'); window.location.href = 'Schedule.php';</script>";
        } else {
    ...
    echo "<script>alert('ID is not set.'); window.location.href = 'Schedule.php';</script>";
}
